...
class GraphData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return [
                ...
                torch.from_numpy(np.stack(self.features_T0[index])).float(),  
                torch.from_numpy(np.stack(self.features_T1[index])).float(),
                torch.from_numpy(np.stack(self.features_T2[index])).float(),
                torch.from_numpy(np.stack(self.features_T3[index])).float(),
                torch.from_numpy(np.stack(self.features_T4[index])).float(),

...
class GraphConv(Module):
    ...
    def forward(self, myCollection):  
        ...
        assert len(input_onestamp.shape) == 3, '输入的数据不对'
...
class GCN(nn.Module):
    ...
    def loss(self, pred, label_sourceNum, label_hot=None, prob_pred=None, γ=None, GCN_models=None, RU_model=None):  
        ...
        return loss_total
    def weight_loss(self, pred, label_sourceNum, label_hot=None, prob_pred=None):
        B = pred.shape[0]
        loss_total = torch.tensor([0.], ).to(args.device)
        weight_I = 1-(label_sourceNum.shape[1] / pred.shape[2])
        weight_S = label_sourceNum.shape[1] / pred.shape[2]
        for i in range(B):
            pred_S_one_B = pred[i, 0, :]  
            pred_I_one_B = pred[i, 1, :]  
            labels_one_B = label_sourceNum[i, :]  
            loss_total = loss_total - (weight_I * sum(label_hot[i] * torch.log(pred_I_one_B)) +\
                                        weight_S * sum((1-label_hot[i]) * torch.log(pred_S_one_B))) / (pred.shape[2])
        loss_total2 = torch.tensor([0.], ).to(args.device)
        for i in range(B):
            minus = prob_pred.squeeze(-1)[i]-label_hot[i]
            minus_I = minus * label_hot[i]
            minus_S = minus * (1 - label_hot[i])
            loss_total2 = loss_total2 + sum(minus_I * minus_I) * weight_I + sum(minus_S * minus_S) * weight_S
        loss_total2 = loss_total2/args.network_verNum
        loss_total = (loss_total + 2 * loss_total2)/B
        return loss_total
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, h, adj, using_loop =False):  
        ...
        if self.concat:
            ...
            return h_prime
    def forward_highComplex(self, h, adj):

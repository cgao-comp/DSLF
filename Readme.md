...
1. **LSTM_model1 and LSTM_model2:**
   ...
   - Input features size is set to 15 for both models. Here, 15 includes 8 constructed dynamic propagation features (H~1~-H~8~ in paper) and 7 user profiles (H_{9}-H_{15}), the verification status of a user is a binary variable, which takes 2 dimensions).
